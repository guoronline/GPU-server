...
- ##### 信息类型：701
  ...
  ```
  1行：开始节点1、开始节点2、结束节点1、结束节点2、内部节点数、内部裂纹节点数
  2行：输内部点的坐标；
  3行：内部裂纹节点号
  3行：输入夹杂的中心和长短轴和角度。
  ```
  ...
  2,	3
  2.13375,  5.72071, 0.630783, 0.630783,  175.643
  126,      3216,      128,      128,        2,	1
  1.50479,  5.76863,  1.53433,  5.52429,
  1
  2.13375,  5.72071, 0.630783, 0.630783,  175.643
  ```

  注：这个5号单元，存在内部裂纹：一界面是第2和第3个内部点断裂；二界面是进入点和内部第1个点断裂。
